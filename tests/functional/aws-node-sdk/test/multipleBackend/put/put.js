...
const { describeSkipIfNotMultiple, getAwsRetry, memLocation, fileLocation,
    awsLocationEncryption, s3Backends } = require('../utils');
...
describe('MultipleBackend put object', function testSuite() {
    ...
    withV4(sigCfg => {
        ...
        describeSkipIfNotMultiple('with set location from "x-amz-meta-scal-' +
            ...
            s3Tests.forEach(s3Type => {
                const { s3Location, s3LocationEncryption } = s3Backends[s3Type];
                ...
                `to ${s3Type} with versioning enabled`, done => {
                    ...
                    async.waterfall([
                        ...
                        (eTag, next) => getAwsSuccess(key, correctMD5,
                            s3Location, s3Type, () => next()),
                    ], done);
                });
                        ...
                        return s3.putObject(params, err => {
                            ...
                            (err, res) => {
                                ...
                                assert.strictEqual(
                                    ...
                                    fileLocation);
                                return getAwsError(key, 'NoSuchKey', s3Type,
                                    done);
                            });
                        });
                    });
