...
const { describeSkipIfNotMultiple, getAwsRetry, awsLocation, gcpLocation,
    awsLocationEncryption, gcpLocationEncryption,
    memLocation, fileLocation } = require('../utils');
...
function getAwsSuccess(key, s3Type, awsMD5, location, cb) {
...
function getAwsError(key, s3Type, expectedError, cb) {
...
function awsGetCheck(objectKey, s3MD5, awsMD5, location, s3Type, cb) {
    ...
    function s3GetCallback(err, res) {
        ...
        return getAwsSuccess(objectKey, s3Type, awsMD5, location, cb);
    });
}
    ...
    withV4(sigCfg => {
        ...
        'location-constraint" header', function describe() {
            ...
            s3Backends.forEach(backend => {
                const { s3Type, s3Location, s3LocationEncryption } = backend;
                ...
                'object to ${s3Type}`, done => {
                    ...
                    return s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected success, ' +
                            `got error ${err}`);
                        return getAwsSuccess(key, s3Type, correctMD5,
                            s3Location, () => done());
                    });
                });

                it(`should put an object to ${s3Type} with encryption`,
                done => {
                    const key = `somekey-${Date.now()}`;
                    const params = { Bucket: bucket, Key: key,
                        Body: body,
                ...
                'to ${s3Type} with versioning enabled`, done => {
                    ...
                    async.waterfall([
                        ...
                        (eTag, next) => getAwsSuccess(key, s3Type, correctMD5,
                            s3Location, () => next()),
                    ], done);
                });
                ...
                skipIfGcp(`should put a large object to ${s3Type}`, done => {
                    ...
                    return s3.putObject(params, err => {
                        assert.equal(err, null, 'Expected sucess, ' +
                            `got error ${err}`);
                        return awsGetCheck(key, bigS3MD5, bigAWSMD5,
                            s3Location, s3Type, () => done());
                    });
                });

                it(`should put objects with same key to ${s3Type}, then file ` +
                ', and object should only be present in file', done => {
                    const key = `somekey-${Date.now()}`;
                    const params = { Bucket: bucket, Key: key,
                        Body: body,
                        Metadata: { 'scal-location-constraint': s3Location } };
                ...
                ', and object should only be present on AWS', done => {
                    const key = `somekey-${Date.now()}`;
                    const params = { Bucket: bucket, Key: key,
                        Body: body,
                        Metadata: {
...
() => {
    withV4(sigCfg => {
        ...
        s3Backends.forEach(backend => {
            const { s3Type, s3Location } = backend;
            it(`should put an object to ${s3Type} with no ` +
            'location header', done => {
                process.stdout.write('Creating bucket\n');
                return s3.createBucket({ Bucket: bucket,
                    CreateBucketConfiguration: {
